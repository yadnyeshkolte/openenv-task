...
# Add api_debug_env to path so imports work from repo root
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "api_debug_env"))
...
You are an expert API debugging agent. Diagnose and fix broken API integrations.
...
Strategy:
1. inspect_logs on each service to find error patterns
2. inspect_config to understand broken settings
3. inspect_endpoint to see actual error responses
4. submit_fix with corrected configuration values

For nested config keys use dot notation: {"headers.Authorization": "Bearer <token>"}
...
def get_model_action(client: OpenAI, obs: ApiDebugObservation, step: int, messages: List[Dict]) -> ApiDebugAction:
...
async def run_task(task_id: str, client: OpenAI) -> tuple:
    ...
    try:
        ...
        score = env.grade()  # already clamped to (0.001, 0.999) in grade()
...
async def main() -> None:
    ...
    for task_id in get_all_task_ids():  # ["easy", "medium", "hard"]
